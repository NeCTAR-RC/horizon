...
def _permission_to_header(is_public):
    headers = {}
    if is_public:
...
def swift_create_container(request, name, is_public=False):
    if swift_container_exists(request, name):
        raise exceptions.AlreadyExists(name, 'container')
    headers = _permission_to_header(is_public)
    swift_api(request).put_container(name, headers=headers)
    return Container({'name': name})


def swift_update_container(request, name, is_public=False):
    headers = _permission_to_header(is_public)
