...
class SecurityGroupsViewTests(test.TestCase):
    ...
    def _add_security_group_rule_fixture(self, ethertype='IPv4', cidr=True,
                                         security_group=None,
                                         ip_protocol=True):
        sec_group = self.security_groups.first()
        ...
        if security_group is True:
            security_group = u'%s' % sec_group.id
    ...
    def test_detail_add_rule_securitygroup_with_invalid_unused_fields(self):
        sec_group, rule = self._add_security_group_rule_fixture(
            cidr=None, ethertype='', security_group=True)
        self.mox.ReplayAll()
