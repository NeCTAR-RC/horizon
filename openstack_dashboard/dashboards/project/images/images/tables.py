...
def image_categories():
    ...
    for propval in filter_properties():
        pv = filter_property_name(propval)
        text_map[pv] = propval['text']
...
def categorize_image(im, user_tenant_id):
    ...
    for propval in filter_properties():
        prop = propval.get('property')
        if prop and im.properties.get(prop) == propval.get('value'):
            categories.append(filter_property_name(propval))
