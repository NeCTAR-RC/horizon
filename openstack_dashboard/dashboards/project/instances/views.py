...
class IndexView(tables.PagedTableMixin, tables.DataTableView):
    ...
    def _get_images(self):
        ...
        try:
            ...
            for image in api.glance.image_list_detailed(self.request)[0]:
                images[str(image.id)] = image.name

            # Use cache for community image listing
            community_images = cache.get('community_images', {})
            if not community_images:
                for image in api.glance.image_list_detailed(self.request, filters={'visibility': 'community'})[0]:
